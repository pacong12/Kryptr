...
| `nx fork-test-sepolia @kryptr/contracts`   | `forge test --fork-url $RPC_URL_BASE_SEPOLIA` | rehearsal stage 1 (84532), nightly/release only |
| `nx fork-test-robinhood @kryptr/contracts` | `forge test --fork-url $RPC_URL_ROBINHOOD`    | rehearsal stage 2 (46630), nightly/release only |
| `nx manifests @kryptr/contracts`           | `node tools/validate-manifests.mjs`           | fail-closed manifest schema check               |
| `nx canonicalize @kryptr/contracts`        | `node tools/canonicalize.mjs --self-test`     | RFC 8785 canonicalizer vectors (G5 contentHash) |
