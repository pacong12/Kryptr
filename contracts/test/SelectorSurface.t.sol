...
contract SelectorSurfaceTest is LaunchTestBase {
    ...
    /// EIP-1967 implementation/admin/beacon slots are zero on FACTORY,
    /// template, and a sample clone (P-2 covers all three; no proxy-storage
    ...
    function test_eip1967Slots_zero() public view {
        ...
        assertEq(vm.load(address(factory), implSlot), bytes32(0));
        assertEq(vm.load(address(factory), adminSlot), bytes32(0));
        assertEq(vm.load(address(factory), beaconSlot), bytes32(0));
