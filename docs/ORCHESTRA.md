...
Skills live in `.agents/skills/` (mirrored to `.opencode/`, `.cursor/`,
`.github/`). Three groups:

1. **`kryptr-*`** — our own operating skills; required reading.
2. **`nx-*`, `link-workspace-packages`, `monitor-ci`** — generated by Nx.
3. **Community skills** — installed from the internet, audited before
   install (markdown-only content accepted; nothing with executable
   payloads):

| Skill | Source | Use for |
|---|---|---|
| `tdd` | mattpocock/skills | red-green loop for api + UI logic |
| `code-review` | mattpocock/skills | reviewing PRs before conductor merge |
| `improve-codebase-architecture` | mattpocock/skills | refactor passes |
| `domain-modeling` | mattpocock/skills | shared-types evolution |
| `codebase-design` | mattpocock/skills | module layout decisions |
| `diagnosing-bugs` | mattpocock/skills | incident debugging |
| `resolving-merge-conflicts` | mattpocock/skills | rebase fallout |
| `react-best-practices` | vercel-labs/agent-skills | backoffice (Next.js) |
| `composition-patterns` | vercel-labs/agent-skills | backoffice component design |
| `web-design-guidelines` | vercel-labs/agent-skills | UX review, both apps |

Rule: a new community skill needs conductor audit (no scripts, no network
calls, no secret access) before it enters the mirrors.
