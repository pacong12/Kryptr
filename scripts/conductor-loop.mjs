...
const AGENTS      = ['vault', 'face', 'deck', 'ops', 'web3', 'reviewer', 'contracts', 'qa', 'redteam'];
...
function herdrList() {
  try {
    const out = execSync('herdr agent list', { encoding: 'utf8', timeout: 5000 });
...
function herdrRead(name) {
  try {
    ...
    return out
      ...
      .split('\n')
      .map(l => l.trim())
      .filter(l => l && !l.startsWith('π') && !l.startsWith('◫') && l.length > 3)
      .slice(-6)
      .join(' | ');
...
function promptTargetAgent(agentName, message) {
  try {
    ...
    console.log(`[conductor-loop] auto-prompted @${agentName} for incoming tektok`);
  } catch (e) {
    console.error(`[conductor-loop] failed to prompt @${agentName}:`, e.message);
  }
}
...
async function poll() {
  ...
  for (const a of agents) {
    ...
    if (prev === 'working' && (status === 'idle' || status === 'done')) {
      ...
      await irc('conductor-loop', 'conductor', `SELESAI: ${name} sudah idle. Ambil action.`);
...
sub.on('message', (_, raw) => {
  try {
    ...
    if (msg.to && AGENTS.includes(msg.to) && msg.to !== 'conductor' && msg.to !== msg.from) {
